...
static void check_key(const std::string& key) {
	if (key.size() != 8) throw std::runtime_error("size of key must be 8.");
}

std::string randomkey() {
	static bool _r = (srandom(time(NULL)), true);
	char tmp[8];
	int i;
	for (i=0;i<8;i++) {
		tmp[i] = random() & 0xff;
...
std::string des_encode(const std::string& key, const std::string& plaintext) {
	...
	std::string r(buffer, buffer+chunksz);
	if (chunksz > SMALL_CHUNK) {
		delete[] buffer;
	}
	return r;
...
std::string des_decode(const std::string& key, const std::string& ciphertext) {
	check_key(key);
	uint32_t ESK[32];
	des_main_ks(ESK, (uint8_t *)const_cast<char *>(&*key.begin()));
	uint32_t SK[32];
	int i;
	for( i = 0; i < 32; i += 2 ) {
		SK[i] = ESK[30 - i];
	...
	size_t textsz = ciphertext.size();
	const uint8_t *text = (const uint8_t *)ciphertext.data();
	if ((textsz & 7) || textsz == 0) {
		throw std::runtime_error("Invalid des crypt text length");
	}
	uint8_t tmp[SMALL_CHUNK];
	uint8_t *buffer = tmp;
	if (textsz > SMALL_CHUNK) {
		buffer = new uint8_t[textsz];
	}
	...
	if (padding > 8) {
		...
		throw std::runtime_error("Invalid des crypt text");
	}
	std::string r(buffer, buffer+textsz-padding);
	if (textsz > SMALL_CHUNK) {
		delete[] buffer;
	}
	return r;
...
std::string hashkey(const std::string& what) {
	size_t sz = what.size();
	const char * key = what.data();
	uint8_t realkey[8];
	Hash(key,(int)sz,realkey);
	return std::string(realkey, realkey+8);
}
...
std::string tohex(const std::string& what) {
	...
	std::string r(buffer, buffer+sz*2);
	if (sz > SMALL_CHUNK/2) {
		delete[] buffer;
	}
	return r;
...
std::string fromhex(const std::string& what) {
	...
	std::string r(buffer, buffer+i/2);
	if (sz > SMALL_CHUNK * 2) {
		delete[] buffer;
	}
	return r;
...
static void
read64(const std::string& k1, const std::string& k2, uint32_t xx[2], uint32_t yy[2]) {
	check_key(k1);
	check_key(k2);
	const uint8_t *x = (const uint8_t *)k1.data();
	const uint8_t *y = (const uint8_t *)k2.data();
...
std::string hmac64(const std::string& k1, const std::string& k2) {
	uint32_t x[2], y[2];
	read64(k1, k2, x, y);
	uint32_t result[2];
	hmac(x,y,result);
...
std::string dhexchange(const std::string& hmac_x) {
	check_key(hmac_x);
	const uint8_t *x = (const uint8_t *)hmac_x.data();
	uint32_t xx[2];
	xx[0] = x[0] | x[1]<<8 | x[2]<<16 | x[3]<<24;
	xx[1] = x[4] | x[5]<<8 | x[6]<<16 | x[7]<<24;
...
std::string base64_decode(const std::string& what) {
	...
	for (i=0;i<sz;) {
		...
		case 1:
			if (c[3] != -2 || (c[2] & 3)!=0) {
				if (decode_sz > SMALL_CHUNK) {delete [] buffer;}
				throw std::runtime_error("Invalid base64 text");
			}
